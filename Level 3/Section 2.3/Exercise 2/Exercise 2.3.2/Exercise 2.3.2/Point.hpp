...
public:
	...
	Point(double x, double y);	//Constructor that accepts x - and y - coordinates so we can create a point with the right values without using the set functions

	double GetX() const;		//Getter function for the x-coordinate
	double GetY() const;		//Getter function for the y-coordinate
	...
	double Distance(const Point &p);	// Calculate the distance between two points. Now passing by reference with const point
};
#endif