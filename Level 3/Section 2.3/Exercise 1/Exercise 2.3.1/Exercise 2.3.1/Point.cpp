...
{
	m_x = p.GetX();					//Copy x-coordinate
	m_y = p.GetY();					//Copy y-coordinate
...
Point::Point(double x, double y)	//Constructor that accepts x - and y - coordinates so we can create a point with the right values without using the set functions
{
	m_x = x;
	m_y = y;
