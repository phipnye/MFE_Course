//Exercise 4.2.2 asks us to build onto exercise 4.2.1.b by adding a NumericArray class, using generic inheritance, so types that support numeric operations can be used
...
using namespace std;
using namespace PhilipNye::Containers;
...
int main()
{
	...
	//With Numeric Arrays we are assuming the elements are numeric so that they can be converted into the form of double for the DotProduct() function
	//A numeric array of points would work but would not be able to perform the DotProduct() function since there's no suitable function to convert a point to a double
